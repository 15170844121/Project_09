<template>
  <div>
    ...
    <!-- 表格组件
         ...
         align:标题的对齐方式
         prop:对应列内容的字段名
         注意： elementUI 当中的 table 组件 展示的数据 是以一列一列进行展示数据
    -->
    ...
    <!--
      分页器
       当前第几页 数据总条数 每一页展示的条数 联连续页码
        @size-change=""
       @current-change=""

         currrent-page：代表的是当前第几页
         total：代表翻页器一共需要展示数据的条数
    ...
    />
    <!-- 对话框
    :visible.sync 控制对话框显示 与隐藏
     Form 组件提供了表单验证的功能，只需要通过 rules 属性传入约定的验证规则，并将 Form-Item 的 prop 属性设置为需校验的字段名即可
    -->
    <el-dialog :title="tmForm.id?'修改品牌':'添加品牌'" :visible.sync="dialogFormVisible">
      <!-- form 表单 :model 属性 这个属性的作用hi是 把表单的数据手机到那个对象的身上 将来表单验证 也需要这个属性 -->
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        // 品牌名称的验证规则
        tmName: [
          //  required  必须要校验的字段 与前面 * 有关  message 提示信息 trigger 用户行为设置 （事件的设置）
  ...
  methods: {
    async getPageList(pager = 3) {
      this.page = pager
      // 解构出参数
      const { page, limit } = this
      // 获取品牌列表的接口
      //  当你向服务器发请求的时候 这个函数需要单参数 因此在 data 当中初始化两个字段 代表给服务器 传递参数
    ...
    addOrUpdateTradeMark() {
      ...
      this.$refs.ruleForm.validate(async(success) => {
        if (success) {
          ...
          const result = await this.$API.trademark.reqAddOrUpdateTradeMake(this.tmForm)
          if (result.code === 200) {
            //  弹出信息  添加品牌成功 修改品牌成功
            this.$message({
              type: 'success',
            ...
            this.getPageList(this.tmForm.id ? this.page : 3)
          }
        } else {
